...
def historical_To_MACD(inputfile , outputfilename):
    ...
    def dFEdittingForMACD(df):
        ...
        df['EMA12'] = df.close.ewm(span=12).mean()  # EMA OF LAST 12 DAYS on close price
        df['EMA26'] = df.close.ewm(span=26).mean()  # EMA OF LAST 26 DAYS on close price
        df['MACD'] = df.EMA12 - df.EMA26            # MACD = EMA12 - EMA26
        df['signal'] = df.MACD.ewm(span=9).mean()   # SIGNAL = EMA of last 9 days of MACD
